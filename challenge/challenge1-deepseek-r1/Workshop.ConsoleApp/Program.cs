...
var kernel = Kernel.CreateBuilder()
                   ...
                   .AddOpenAIChatCompletion(
                        ...
                        serviceId: "github")
                    .AddOpenAIChatCompletion(
                        modelId: config["DeepSeekR1:Models:ModelId"]!,
                        openAIClient: client,
                        serviceId: "DeepSeek-R1"
                    )
                   .Build();
...
while (true)
{
    ...
    Console.WriteLine();
    Console.WriteLine("--- Response from DeepSeek-R1 ---");
    var responseDeepSeek = kernel.InvokePromptStreamingAsync(
            promptTemplate: input,
            arguments: new KernelArguments(new PromptExecutionSettings() { ServiceId = "DeepSeek-R1" }));
    await foreach (var content in responseDeepSeek)
    {
        await Task.Delay(20);
        message += content;
        Console.Write(content);
